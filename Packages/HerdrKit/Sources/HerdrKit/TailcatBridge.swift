...
///
/// This is a thin HerdrKit-side coordinator over `HerdrTailcat.TailcatBridge`:
/// it maps a device to its deterministic local socket path and reads the
/// connection token from the Keychain, then hands both to the in-process
/// bridge. The bridge serves the remote herdr on that socket, so `SocketRPC`,
/// the event stream, and terminal attach reach it exactly as the SSH forward
/// lets them. A process-wide manager is required because terminal attach builds
/// its command from a fresh `HerdrService` (see `AttachTerminalView`) and
/// cannot reach the session's service; the deterministic socket path lets it
/// name the socket without any shared state.
public actor TailcatBridgeManager {
    ...
    public func ensureUp(deviceID: UUID) async throws -> String {
        guard let token = TailcatTokenStore.existingToken(for: deviceID) else {
